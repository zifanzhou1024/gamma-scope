...
async function confirmReplayImportUrl(context: ConfirmRouteContext): Promise<string> {
  const { importId } = await context.params;
  return `${apiBaseUrl()}${REPLAY_IMPORTS_PATH}/${encodeURIComponent(importId)}/confirm`;
}
...
export async function POST(request: Request, context: ConfirmRouteContext) {
  ...
  try {
    const upstreamResponse = await fetch(await confirmReplayImportUrl(context), {
      method: "POST",
      cache: "no-store",
      headers: {

...
describe("GET /api/spx/0dte/snapshot/latest", () => {
  ...
  it("proxies to FastAPI and preserves upstream body, status, and content type", async () => {
    const fetcher = vi.fn(async () => textResponse(JSON.stringify({ ok: true }), {
      status: 202,
      headers: {
        "Content-Type": "application/vnd.gammascope.snapshot+json"
      }
    }));
    ...
    const response = await GET(new Request("http://localhost/api/spx/0dte/snapshot/latest"));

    expect(response.status).toBe(202);
    await expect(response.text()).resolves.toBe(JSON.stringify({ ok: true }));
    expect(response.headers.get("Content-Type")).toBe("application/vnd.gammascope.snapshot+json");
    expect(response.headers.get("Cache-Control")).toBe("no-store");
    expect(fetcher).toHaveBeenCalledWith("http://fastapi.test/api/spx/0dte/snapshot/latest", {
      cache: "no-store",
      headers: {
  ...
  it("returns no-store 502 JSON when the upstream fetch fails", async () => {
    ...
    await expect(response.json()).resolves.toEqual({ error: "Snapshot API unavailable" });
    expect(response.headers.get("Cache-Control")).toBe("no-store");
  });
});
